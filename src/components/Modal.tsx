...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, icon, iconColor = 'blue', maxWidth = 'md' }) => {
  ...
  return (
    <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 animate-fade-in'>
      <div className={`bg-white rounded-2xl p-8 ${widthClasses[maxWidth]} w-full shadow-2xl relative animate-slide-in`}>
        <button onClick={onClose} className='absolute top-4 right-4 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full p-2 transition-colors'>
